...
static unsigned int lcm_opt_error_flag = FALSE;
static unsigned int lcm_opt_check_flag = FALSE;
static unsigned int lcm_opt_reg_dump_count = 0;
...
//{0x00,1, {0x00}},//use factory otp target
//{0xD9,1, {0xA2}},
...
static struct LCM_setting_table lcm_initialization_setting_default_opt[] = {
#if 0
{0x00,1, {0x00}},
{0xFF,3, {0x12,0x83,0x01}},
...
/// 
    ...
    data_array[0] = 0x00042902;
    data_array[1] = 0x018312FF;
    dsi_set_cmdq(&data_array, 2, 1);//EXTC = 1      {0xFF,3, {0x12,0x83,0x01}},

    data_array[0] = 0x80002300;              // {0x00,1, {0x80}},
    dsi_set_cmdq(&data_array, 1, 1);	

    data_array[0] = 0x00032902;        
    data_array[1] = 0x008312FF; 
    dsi_set_cmdq(&data_array, 2, 1);  //   {0xFF,2, {0x12,0x83}},  //Orise mode enable

    data_array[0] = 0xC6002300;
    dsi_set_cmdq(&data_array, 1, 1);    //{0x00,1, {0xC6}},

     data_array[0] = 0x03B02300;
    dsi_set_cmdq(&data_array, 1, 1);    //{0xB0,1, {0x03}},
...
//LINE<JIRA_ID><DATE20140613><BUG_INFO>zenghaihui
   //array[0] = 0x00043700;// read id return 4 bytes,version and id

   data_array[0] = 0x03002300;
   dsi_set_cmdq(&data_array, 1, 1);    //{0x00,1, {0xC6}},

   read_reg_v2(0xf1, buffer, 1);    
   LCM_DBG("lcm_compare_id read 0xf1 buffer[0]=%x",  buffer[0]);

   if(buffer[0] == 0)
    {
        lcm_opt_error_flag = TRUE;
    }
   else
    {
        lcm_opt_error_flag = FALSE;
    }

    lcm_opt_check_flag = TRUE;
   ...
   read_reg_v2(0xd9, buffer, 1);    
      LCM_DBG("lcm_compare_id read 0xd9 buffer[0]=%x", buffer[0]);
...
static void lcm_init(void)
{
    ...
    SET_RESET_PIN(1);
    MDELAY(10);
    if(lcm_opt_error_flag == TRUE)
    {
        LCM_DBG("lcm_init: lcm_initialization_setting_default_opt");
        
        push_table(lcm_initialization_setting_default_opt, sizeof(lcm_initialization_setting_default_opt) / sizeof(struct LCM_setting_table), 1);
    }
    else
    {
        LCM_DBG("lcm_init: lcm_initialization_setting");
...
//LINE<JIRA_ID><DATE20140613><BUG_INFO>zenghaihui
   ...
   read_reg_v2(0xf1, buffer, 1);    
   LCM_DBG("lcm_compare_id: lcm_dump_reg read 0xf1 buffer[0]=%x",  buffer[0]);
   ...
   read_reg_v2(0xd9, buffer, 1);    
      LCM_DBG("lcm_compare_id: lcm_dump_reg read 0xd9 buffer[0]=%x", buffer[0]);
...
static void lcm_suspend(void)
{
    ...
    if(lcm_opt_reg_dump_count < 10)
    {
        lcm_opt_reg_dump_count++;
        lcm_dump_reg();
    }
...
{
    .name			= "otm1283a_hd720_dsi_vdo_dijing",
